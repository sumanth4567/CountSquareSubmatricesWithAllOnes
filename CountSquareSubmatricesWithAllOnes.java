...
public class CountSquareSubmatriceswithAllOnes {
    ...
    public static void main(String[] args) {
        ...
        int m=sc.nextInt();
        int mat[][]=new int[n][m];
        System.out.println("Enter "+(n*m)+" numbers with values 0 or 1 into the array :");
        for(int i =0;i<n;i++){
            for(int j=0;j<m;j++){
                mat[i][j]=sc.nextInt();
            }
        }
        int result=csswao.countSquares(mat);
        System.out.println("The nuber of squares in the given matrix containing 1's are :"+result);
    }
}
